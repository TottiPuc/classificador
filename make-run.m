...
[x4,fs] = wavread('~/classificador/noiseComplete/factory1.wav');
...
if d == 1
	...
	[X1E, X1M, X1D] = extraction(x1,fs,d);
	[X2E, X2M, X2D] = extraction(x2,fs,d);	
	[X3E, X3M, X3D] = extraction(x3,fs,d);
	[X4E, X4M, X4D] = extraction(x4,fs,d);
	l = length(X1E);
	labelX1E = ones(l,1);
	labelX2E = 2*ones(l,1);
	labelX3E = 3*ones(l,1);
	...
	Y = [[X1E,X1M,X1D,labelX1E]; [X2E,X2M,X2D,labelX2E]; [X3E,X3M,X3D,labelX3E]; [X4E,X4M,X4D,labelX4E]];
...
elseif d == 2
	...
	l = length(X1E);
        ...
        X = [[X1E,X1M,X1D,labelX1E]; [X2E,X2M,X2D,labelX2E]; [X3E,X3M,X3D,labelX3E]; [X4E,X4M,X4D,labelX4E]];
	X1 = [[X1M,X1D]; [X2M,X2D]; [X3M,X3D]; [X4M,X4D]];
